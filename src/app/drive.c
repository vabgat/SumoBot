 ...
 * and create a macro to get the corresponding primary directions for every direction and
 * inverse the speeds when its not the primary direction. */
#define DRIVE_PRIMARY_DIRECTIONS(dir) (dir - MODULO_2(dir))
static_assert(DRIVE_PRIMARY_DIRECTIONS(DRIVE_DIR_REVERSE) == DRIVE_DIR_FORWARD);
...
void drive_set(drive_dir_e direction, drive_speed_e speed) {
    drive_dir_e primary_direction = DRIVE_PRIMARY_DIRECTIONS(direction);

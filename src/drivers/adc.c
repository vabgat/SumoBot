...
void        adc_init(void) {
    ASSERT(!initialized);
    adc_pins = io_adc_pins(&adc_pins_cnt);

    uint8_t adc10ae0 = 0;
    uint8_t last_idx = 0;
    for (uint8_t i = 0; i < adc_pins_cnt; i++) {
     ...
     * MSC: Mulitple sample conversion
    ...
    /* Use data transfer controller (DTC) to trasfer data DMA-style from
     * the sample channels and interrupt afterwards. Note, CONSEQ_1 iterates
     * the chaneels contiguosly from last idx to 0. */
...
void adc_get_channel_values(adc_channel_values_t values) {
     ...
     * here as then ADC stops workign after a whle, so as a workaround disable
     * interrupts globally. */
    _disable_interrupts();
    for (uint8_t i = 0; i < adc_pins_cnt; i++) {
